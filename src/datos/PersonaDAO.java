...
public class PersonaDAO implements IPaginadoInterface<Persona> {
    ...
    public List<Persona> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        try {
            ...
            //LIMIT PARA INDICAR DESDE QUE REGISTRO VOY A EMPEZAR A MOSTRAR Y EL OTRO SIGO ? INDICA HASTA QUE REGISTRO
    ...
    public List<Persona> listarTipo(String texto, int totalPorPagina, int numPagina, String tipoPersona) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                registros.add(new Persona(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getBoolean(9)));//LLENANDO LOS PARAMETROS DEL CONSTRUCTOR DE LA CLASE ARTICULO
            }
            ps.close();
            rs.close();
        } catch (SQLException e) {
    ...
    public boolean insertar(Persona obj) {
        resp = false;
        try {
            ps = CON.conectar().prepareStatement("INSERT INTO persona (tipo_persona,nombre,tipo_documento,num_documento,direccion,telefono,email,activo) VALUES (?,?,?,?,?,?,?,1)");//UNO PORQUE EN LA BD EL CAMPO ACTIVO ES UN BIT SOLO RECIBE UNOS Y 0 Y RECORDEMOS QUE POR DEFECTO EL ESTA EN 1
