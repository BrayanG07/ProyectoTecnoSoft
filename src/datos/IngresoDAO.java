...
public class IngresoDAO implements IIngresoInterface<Ingreso, DetalleIngreso> {
    ...
    public List<Ingreso> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        try {
            ps = CON.conectar().prepareStatement("SELECT i.id, i.usuario_id, u.nombre as usuario_nombre, i.persona_id ,p.nombre as persona_nombre, i.tipo_comprobante, i.serie_comprobante,i.num_comprobante as numero_de_comprobante, i.fecha, i.impuesto,i.total, i.estado FROM ingreso i INNER JOIN persona p ON i.persona_id = p.id INNER JOIN usuario u ON i.usuario_id = u.id WHERE i.num_comprobante LIKE ? ORDER BY i.id ASC LIMIT ?,?");
...
//          import com.mysql.jdbc.Statement;
            ...
            rs = ps.getGeneratedKeys();//OBTENIENDO LA PK GENERADA    
            int idGenerado = 0;
            if (rs.next()) {
    ...
    public List<Venta> consultaFechas(Date fechaInicio, Date fechaFin) {
        ...
        try {
            ps = CON.conectar().prepareStatement("SELECT i.id, i.usuario_id, u.nombre as usuario_nombre, i.persona_id ,p.nombre as persona_nombre, i.tipo_comprobante, i.serie_comprobante,i.num_comprobante as numero_de_comprobante, i.fecha, i.impuesto,i.total, i.estado FROM ingreso i INNER JOIN persona p ON i.persona_id = p.id INNER JOIN usuario u ON i.usuario_id = u.id WHERE i.fecha>=? AND i.fecha<=?");
