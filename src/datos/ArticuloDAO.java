...
public class ArticuloDAO implements IPaginadoInterface<Articulo> {
    ...
    public Articulo obtenerArticuloCodigoVenta(String codigo) {
        ...
        try {
            ...
            if (rs.first()) {//SI OBTENGO UN RESULTADO
                art = new Articulo(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4), rs.getInt(5));
            }
            ps.close();
            rs.close();
    ...
    public boolean insertar(Articulo obj) {
        resp = false;
        try {
            ps = CON.conectar().prepareStatement("INSERT INTO articulo (categoria_id,codigo,nombre,precio_venta,stock,descripcion,imagen,activo) VALUES (?,?,?,?,?,?,?,1)");//UNO PORQUE EN LA BD EL CAMPO ACTIVO ES UN BIT SOLO RECIBE UNOS Y 0 Y EL QUE POR DEFECTO ESTA EN 1
