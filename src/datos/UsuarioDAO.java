...
public class UsuarioDAO implements IPaginadoInterface<Usuario> {
    ...
    public List<Usuario> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        try {
            //RECORDATORIO: PROBAR MAÑANA ESTE QUERY EN WORKBENCH DA ERROR, VERIFICAR
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                registros.add(new Usuario(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9), rs.getString(10), rs.getBoolean(11)));//LLENANDO LOS PARAMETROS DEL CONSTRUCTOR DE LA CLASE ARTICULO
            }
            ps.close();
            rs.close();
    ...
    public boolean insertar(Usuario obj) {
        resp = false;
        try {
            ps = CON.conectar().prepareStatement("INSERT INTO usuario (rol_id,nombre,tipo_documento,num_documento,direccion,telefono,email,clave,activo) VALUES (?,?,?,?,?,?,?,?,1)");//1 PORQUE EN LA BD EL CAMPO ACTIVO ES UN BIT SOLO RECIBE UNOS Y 0 QUE POR DEFECTO ESTA EN 1
